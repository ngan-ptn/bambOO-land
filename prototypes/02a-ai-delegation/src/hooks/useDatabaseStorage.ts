...
interface UseDatabaseStorageReturn {
  ...
  addFoodForUser: (userId: string, food: FoodItem, portion: PortionSize) => Promise<LogEntry | null>
...
export function useDatabaseStorage(): UseDatabaseStorageReturn {
  ...
  // Add food for a specific user (used for partner logging)
  const addFoodForUser = useCallback(
    async (userId: string, food: FoodItem, portion: PortionSize): Promise<LogEntry | null> => {
      const nutrition = food.portions[portion]
