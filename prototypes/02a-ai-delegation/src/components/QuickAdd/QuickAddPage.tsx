...
import { PortionPicker, type PortionSelection } from './PortionPicker'
import { Toast } from '@/components/common/Toast'
import { Dashboard } from '@/components/Dashboard'
import { ManualEntryModal } from '@/components/ManualEntry/ManualEntryModal'
import { useDatabaseStorage, useFoodSearch, usePartner } from '@/hooks'
...
export function QuickAddPage({}: QuickAddPageProps) {
  ...
  const { recentItems, removeLog, dailySummary, goals, addFood, addFoodForUser, todayLogs } = useDatabaseStorage()
  const { currentUser, invalidateFavorites } = useDatabaseContext()
  const { partner, hasPartner, isLoadingPartner } = usePartner()
   ...
   * Supports logging for partner when toggle is ON.
   */
  const handleSelectPortion = useCallback(
    async (selection: PortionSelection) => {
      ...
      trackEvent('favorite_logged', {
        ...
        portion,
        method: 'tap',
        logged_for_partner: logForPartner,
      })
    ...
    [selectedFood, currentUser, addFood, addFoodForUser, invalidateFavorites, partner]
  )
  /**
